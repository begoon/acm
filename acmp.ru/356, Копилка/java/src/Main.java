...
public class Main {
    ...
    private void solve() {
        ...
        for (int i = 0; i < coins.length; ++i) {
            coins[i] = new Coin(is.readInt(), is.readInt());
        }

        int[] dp_max = new int[C + 1];
        int[] dp_min = new int[C + 1];

        dp_max[0] = dp_min[0] = 0;

        for (int i = 1; i <= C; ++i) {
            dp_max[i] = Integer.MIN_VALUE;
            dp_min[i] = Integer.MAX_VALUE;
            for (Coin coin: coins) {
                int j = i - coin.weight;
                if (j < 0) continue;
                if (dp_max[j] != Integer.MIN_VALUE) {
                    dp_max[i] = Math.max(dp_max[i], dp_max[j] + coin.value);
                }
                if (dp_min[j] != Integer.MAX_VALUE) {
                    dp_min[i] = Math.min(dp_min[i], dp_min[j] + coin.value);
                }
            }
        }

        if (dp_max[C] == Integer.MIN_VALUE || dp_min[C] == Integer.MAX_VALUE)
            os.println("This is impossible.");
        else {
            os.print(dp_min[C]);
            os.print(' ');
            os.print(dp_max[C]);
