...
void solve(istream& is, ostream& os) {
  int n;
  is >> n;
  int m[10] = { 46, 68, 79, 48, 390, -1, 170, 26, 13, 42 };
  vector<vector<bignum> > dp(n, vector<bignum>(10, 0));
  int t[10] = { 0, 1, 1, 1, 1, 1, 1, 1, 0, 1 };
  copy(t, t + 10, dp[0].begin());
  for (int t = 1; t < n; ++t) {
    for (int i = 0; i < 10; ++i) {
      if (m[i] == -1) continue;
      if (m[i] < 100)
        dp[t][i] = dp[t - 1][m[i] / 10] + dp[t - 1][m[i] % 10];
      else
        dp[t][i] = dp[t - 1][m[i] / 100] + dp[t - 1][(m[i] / 10) % 10] + dp[t - 1][m[i] % 10];
    }
  }
  bignum ans = accumulate(dp[n - 1].begin(), dp[n - 1].end(), bignum(0));
  os << ans.str() << endl;
}
...
"100\n"
,
"2657396588204099682921354979006480384\n"
);
