...
const int MAXD = 105, DIG = 1, BASE = 10;
...
int main(int argc, char* argv[]) {
  // istream& is = &cin;
  // ifstream is(argc > 1 ? argv[1] : "INPUT0.TXT");
  // ifstream is(argc > 1 ? argv[1] : "INPUT1.TXT");
  ifstream is(argc > 1 ? argv[1] : "INPUT.TXT");
  ...
  vector<vector<bignum> > s(N + 1, vector<bignum>(K + 1, 0));
  for (int i = 0; i <= 9; ++i) s[1][i] = 1;
  for (int n = 2; n <= N; ++n)
    for (int k = 0; k <= K; ++k)
      for (int l = 0; l <= min(k, 9); ++l)
        s[n][k] = s[n][k] + s[n - 1][k - l];
