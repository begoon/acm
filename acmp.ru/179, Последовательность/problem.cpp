...
int f(int i, int j) {
  if (i == 0) return 1;
  return j & 1 ? f(i-1, j/2) + f(i-1, j/2+1) : f(i-1, j/2);
}

int w(int i, int j, vector<vector<int> >& dp) {
  if (i == 0) return 1;
  if (dp[i][j] != -1) return dp[i][j];
  int r = j & 1 ? w(i-1, j/2, dp) + w(i-1, j/2+1, dp) : w(i-1, j/2, dp);
  dp[i][j] = r;
  return r;
}

void solve_complicated(istream& is, ostream& os) {
  int K;
  is >> K;
  int N = g(K);
  int r = 0;
