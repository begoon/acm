...
bool check(const string& a, const string& b, int L) {
  int offset = L - b.length();
  for (int i = offset, j = 0; i < a.length(); ++i, ++j) {
  ...
  while (T--) {
    ...
    if (delta >= 0) {
      ...
      if (check(a, b, L))
        ans += 1;
      int offset = -delta;
      string aa = a.substr(0, offset);
      string bb = b.substr(b.length() - offset, offset);
      if (aa != bb && check(b, a, L))
        ans += 1;
    }
    os << ans << endl;
  }
